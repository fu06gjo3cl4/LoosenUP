...
class DetectViewController: UIViewController , CBCentralManagerDelegate ,CBPeripheralDelegate{
    ...
    @IBOutlet weak var view_detect_time: UIView!{
        didSet{
//            UIViewService.addShadow(view: view_detect_time, color: UIColor.black.cgColor, width: 0.5, height: 0, radius: 5.0, opacity: 0.5)
            UIViewService.SetCornerRadius(view: view_detect_time, cornerRadius: 5.0)
        }
    }
    @IBOutlet weak var view_tired_degree: UIView!{
        didSet{
//            UIViewService.addShadow(view: view_tired_degree, color: UIColor.black.cgColor, width: 10, height: 10, radius: 5.0, opacity: 0.5)
            UIViewService.SetCornerRadius(view: view_tired_degree, cornerRadius: 5.0)
        }
    }
    
    @IBOutlet weak var btn_start_detect: UIButton!{
        didSet{
            UIButtonService.SetBackgroundColor(button: btn_start_detect,color: Const.Main_Color)
            UIViewService.SetCornerRadius(view: btn_start_detect, cornerRadius: 5.0)
        }
    }
    @IBOutlet weak var btn_finish_detect: UIButton!{
        didSet{
            UIButtonService.SetBackgroundColor(button: btn_finish_detect,color: Const.Main_Color)
            UIViewService.SetCornerRadius(view: btn_finish_detect, cornerRadius: 5.0)
        }
    }
...
//        timer = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(DetectViewController.setchart), userInfo: nil, repeats: true)
    ...
    override func viewDidLayoutSubviews() {
        ...
        if(times_viewDidLayoutSubviews == 2){
            UIViewService.addtopborder(view: self.view_lineChart, color: UIColor.black.cgColor, height: 3.0)
            UIViewService.addbottomborder(view: self.view_lineChart, color: UIColor.black.cgColor, height: 3.0)
        }
    }
